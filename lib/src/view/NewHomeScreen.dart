...
import 'package:news/src/view/widget/drawer.dart';
...
class _NewHomeScreenState extends State<NewHomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:
          isLoading // ------------->  Show loading shimmer effect
              ? Shimmer.fromColors(
...
// Yeh widget news list show karega
class NewsItemList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      itemCount: newsApiDataItems.length, // total articles count
      shrinkWrap: true, // scroll issue avoid karne ke liye
      physics:
          ClampingScrollPhysics(), // parent scroll ke sath properly work karega
      itemBuilder: (context, index) {
        final NewsItem = newsApiDataItems[index]; // ek ek news item lena
        ...
        return GestureDetector(
          onTap: () {
            Get.to(
              () => Newsdetailsscreen(
                imagePath:
                    NewsItem.urlToImage ??
                    "", // agar image available hai to dikhana
                heroTag: 'newsImage$index', // hero animation ke liye unique tag
                NewsTitle:
                    NewsItem.title ??
                    "No Title", // agar title nahi hai to default text
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(
                  color: Colors.black12, // halka shadow dene ke liye
                  blurRadius: 5,
                  spreadRadius: 2,
                  offset: Offset(0, 3),
                ),
            ...
            child: Row(
              ...
              children: [
                Hero(
                  tag: 'newsImage$index', // smooth animation ke liye
                  child: ClipRRect(
                    borderRadius: BorderRadius.circular(
                      10,
                    ), // image ke edges round karne ke liye
                    child: Image.network(
                      NewsItem.urlToImage ?? "null", // image URL fetch karna
                      width: 180, // fixed width
                      height: 140, // fixed height
                      fit: BoxFit.cover, // image ko properly adjust karna
                      errorBuilder: (context, error, stackTrace) {
                        return Icon(
                          ...
                          color:
                              Colors
                                  .grey, // agar image nahi mili to broken image icon
                        );
                      },
                    ),
                  ),
